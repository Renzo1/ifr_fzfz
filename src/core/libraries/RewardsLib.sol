...
library RewardsLib {
    ...
    ) internal returns (address[] memory tokens, uint256[] memory _amounts) {
    ...
    function collectBribesInWBERA(
        ...
        address voter,
        uint256 rewardsDuration
    ) internal returns (uint256 amtInfraredBERA, uint256 amtIbgtVault) {
    ...
    function harvestBoostRewards(
        ...
        address voter,
        uint256 rewardsDuration
    ) internal returns (address _vault, address _token, uint256 _amount) {
